...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		if (user != null) {
			...
			if(!"01".equals(user.getUserType())){
				throw new AuthenticationException("msg:该账号没有此权限.");
			}
			byte[] salt = Encodes.decodeHex(user.getPassword().substring(0,16));//盐值
			return new SimpleAuthenticationInfo(new Principal(user), 
					user.getPassword().substring(16), ByteSource.Util.bytes(salt), getName());
		} else {
			logger.error(String.format("该已帐号不存在"));
			throw new AuthenticationException("msg:该已帐号不存在.");
		}
	}
	...
	public static class Principal implements Serializable {
		...
		private String isInit;//手否已初始化 0表示已经改过密码
